...
const deliveryBoyOperations =  {
    ...
    async findOrders(orders,orderStatus){
        ...
        orderSchema.find({orderId:{$in:orderIdArray},orderStatus:{$in:orderStatus}},(err,docs)=> {
            if(err) {
                reject(err);
            }
            else{
    ...
    async fetchPendingOrder(deliveryBoyEmail) {
        ...
        let allOrders = await this.findOrders(allOrderIds,config.pendingOrders);
    ...
    async loginDeliveryBoy(emailId,password){
        ...
        let doc=await this.findByEmailId(emailId);
                    ...
                    if(checkPassword) {
                        ...
                        let rightId=await this.findRightId(roleId.roleId);
